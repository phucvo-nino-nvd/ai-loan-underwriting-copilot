...
export function TopPerformers() {
  return (
    <div className="bg-card/40 border border-border/60 p-5 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-300">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h3 className="text-base font-semibold text-foreground">Top Underwriters</h3>
          <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground mt-1">This month&apos;s leaders</p>
        </div>
      ...
      <div className="space-y-3">
        {performers.map((person, index) => (
          <div
            key={person.name}
            className="group flex items-center justify-between p-3 hover:bg-secondary/50 transition-all duration-200 cursor-pointer animate-in fade-in slide-in-from-right-2"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{person.deals} assessments completed</p>
              </div>
            </div>
