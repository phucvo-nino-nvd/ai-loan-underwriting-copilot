...
/** YYYY-MM-DD, straight off the timestamp — sorts chronologically as a plain string. */
function dayKey(iso: string) {
  return iso.slice(0, 10);
}

function dayLabel(key: string) {
  // Bare YYYY-MM-DD parses as UTC midnight, which lands on the previous day west of Greenwich.
  return new Date(`${key}T00:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric" });
}

const RISK_APPETITE = 0.04; // 4%

export function PdTrendChart({
  assessments,
}: {
  ...
  const data: PDDataPoint[] = useMemo(() => {
    ...
    return [...groups.entries()]
      ...
      .map(([key, records]) => ({
        day: dayLabel(key),
        pd: records.reduce((sum, r) => sum + r.probability, 0) / records.length,
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground mt-1">
            Daily portfolio PD vs risk appetite
          </p>
        </div>
